...
class RepitentesController extends AppController
{
    ...
    public function confirmarAlumnos()
    {
        ...
        $api = $this->Siep->consumeApi("api/v1/repitencia",$apiParams,'POST');

        if(isset($api['error']))
        {
            $apiMsg = $this->Siep->apiHasError($api);
            $this->Session->setFlash("Error al repetir<br>".$apiMsg, 'default', array('class' => 'alert alert-danger'));
            $this->redirect([
                'action' => 'index',
            ...
            foreach ($api['response'] as $id => $item)
            {
                if(isset($item['error']))
                {
                    ...
                    $response[] = $item['legajo_nro']." Repitio con exito";
                }
            }

            $msg = join('<br>',$response);

            $this->Session->setFlash("Repitencia ejecutada <br>".$msg, 'default', array('class' => 'alert alert-success'));
            $this->redirect([
                'action' => 'index',
                'centro_id' => $apiParams['centro_id'],
                'curso_id' => $apiParams['curso_id'],
