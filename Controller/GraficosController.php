...
class GraficosController extends AppController {
	...
	public function index() {
		...
		// Conteo de las titulaciones en Secundarios.
		if ($nivelCentro == 'Común - Secundario') {
			...
			$titulaciones = $this->Titulacion->CentrosTitulacion->find('count', array(
				'conditions' => array(
		...
		$matriculaBaja = $this->Inscripcion->find('count', array(
			'conditions' => array(
				...
				'Inscripcion.ciclo_id' => $cicloIdActual,
				'Inscripcion.tipo_baja !=' => 'NULL'
			))
		);
		// Conteo de promociones.(PENDIENTE: HACERLO CON el campo "promocion_id")
		$matriculaPromociones = $this->Inscripcion->find('count', array(
			'conditions' => array(
				'Inscripcion.centro_id' => $userCentroId,
				'Inscripcion.promocionado !=' => 'NULL',
				'Inscripcion.ciclo_id' => $cicloIdAnterior
			))
		);
		// Conteo de egresos. (PENDIENTE CON el campo repitencia_id) 
		// Conteo de egresos.
		...
		*/
        switch ($nivelCentro) {
			...
			case 'Adulto - Primario':
			case 'Adulto - Secundario':
		...
		$this->set(compact('centroNombre', 'usuarios', 'cursos', 'matricula', 'ingresantes',
		 'matriculaBaja', 'matriculaPromociones', 'matriculaEgresos', 'matriculaPases', 
		 ...
		 'cursosQuintosAnios', 'cursosSextosAnios', 'cursosSeptimosAnios', 'inscripcionesPorHermano', 'inscripcionesComunes', 'inscripcionesPorSituacionSocial',
		 'inscripcionesPorPase', 'bajasSalidosConPase', 'bajasSalidosSinPase', 'bajasPerdidaRegularidad', 
		 'bajasFallecimiento', 'bajasSinEspecificar', 'titulaciones', 'nivelCentro')); 
	} 
	/*
